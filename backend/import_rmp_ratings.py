...
async def import_rmp_ratings():
    ...
    math_possible_paths = [
        ...
        script_dir.parent / "RateMyProfessor.com-Web-Scraper" / "math_professors_with_reviews.json",
    ...
    for rmp_prof in rmp_data:
        ...
        if not instructor:
            ...
            for existing_name, existing_instructor in instructor_by_name.items():
                ...
                if (rmp_name_parts and existing_parts and 
                    ...
                    if (rmp_first == existing_first or 
                        rmp_first.startswith(existing_first) or 
                        existing_first.startswith(rmp_first) or
                        (rmp_first in ["greg", "jim", "jimmy"] and existing_first in ["gregory", "james"]) or
                        (existing_first in ["greg", "jim", "jimmy"] and rmp_first in ["gregory", "james"])):
                        ...
                        if "ted" in name.lower() and "ted" not in existing_name.lower():
                            ...
                            updated_instructor = Professor(
                                ...
                                department=rmp_prof.get('_department') or existing_instructor.department,
                            )
        ...
        if instructor:
            ...
            if 'ahn' in name.lower() and ('ted' in name.lower() or 'tae' in name.lower()):
                instructor_name = 'Ted Ahn'
            elif 'ahn' in instructor.name.lower() and ('tae' in instructor.name.lower() and 'ted' not in instructor.name.lower()):
                instructor_name = 'Ted Ahn'
            ...
            await services.instructor_repository.save(updated_instructor)
            matched += 1
            print(f"✓ Matched: {name} -> {instructor_name} (Rating: {rmp_prof.get('overall_rating')}, Dept: {department})")
        else:
            ...
            matched += 1
            print(f"✓ Created new instructor: {instructor_name} (Rating: {rmp_prof.get('overall_rating')}, Dept: {rmp_prof.get('_department')})")
